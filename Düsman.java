...
public class Düsman {
	...
	private Image düsman;
        ...
        public Düsman() {
		ImageIcon img = new ImageIcon("C://Users//Oğuzhan//Desktop//aaa//gargamel.png");
		düsman = img.getImage();
        ...
        public void degis1(int dx, int dy)
	{
        ...
        for (int adim = 0; adim < Adim; adim++) {
            ...
            YolSec(getTilex() - 1, getTiley(), 1, 0,map);
            YolSec(getTilex() + 1, getTiley(), 1, 0,map);
            YolSec(getTilex(), getTiley() - 1, 1, 0,map);
            YolSec(getTilex(), getTiley() + 1, 1, 0,map);
        ...
        if ((Xdeger > -1) && (Ydeger > -1) && (Xdeger < 14) && (Ydeger < 11)) {
            if (karakter == 0) { 
                if (harita[Xdeger][Ydeger] > mesafe) {
